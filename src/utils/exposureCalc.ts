...
// EV (at ISO 100) used here is proportional to light reaching sensor.
// We compute it as log2(shutter / aperture^2) so that:
// - smaller F (smaller numeric aperture) -> larger brightness (higher EV)
// - larger shutterSeconds (longer exposure) -> larger brightness (higher EV)
export function evAtISO100(aperture: number, shutterSeconds: number): number {
	return Math.log2(shutterSeconds / (aperture * aperture));
}
